...
cd "$ROOT"
EXPECTED_BUNDLE_ID="com.jrod042.casarusticogo"
...
npx expo config --type public --json >/tmp/crg-expo-config.json
node <<'NODE'
const c = require('/tmp/crg-expo-config.json');
if (c.ios?.bundleIdentifier !== 'com.jrod042.casarusticogo') {
